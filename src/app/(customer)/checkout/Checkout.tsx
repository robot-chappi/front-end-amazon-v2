...
const Checkout: FC<{products: IProduct[]}> = ({products=[]}) => {
    ...
    const {mutate} = useMutation(['create order and payment'], () => OrderService.place({
            ...
            <section className={styles.checkout}>
                <div>
                    ...
                    <div className={styles.footer}>
                        ...
                        <Button variant='light' size='lg' className='mt-5 mb-2' onClick={() => mutate()}>Place order</Button>
                    </div>
                </div>
                <div>
                    <Heading className='mb-6 text-2xl'>Recommended products</Heading>
                    <div className={styles.recommended}>
                        {products.filter(product => !items.map(item => item.product.id).includes(product.id)
                        ).slice(0, 2)
                        .map(product => (
