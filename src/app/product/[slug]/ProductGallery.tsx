...
export function ProductGallery({images}: IProductGallery) {
    ...
    return (
        <div>
            <Image src={images[activeIndex]} alt='' width={500} height={500} className='rounded-lg overflow-hidden' priority draggable={false}/>
            <div className='mt-6' style={{width: '500px', overflowX: 'auto', whiteSpace: 'nowrap'}}>
                {images.map((image, index) => (
                    <button key={index} onClick={() => setActiveIndex(index)} className={cn(
